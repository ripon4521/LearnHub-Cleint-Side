...
//   const totalPrice = enrollment.reduce((sum, item) => sum + item.price, 0).toFixed(2);
    ...
    const handleTeacherApprove=(item)=>{
         axiosSecure.post('/courses', item)
        .then(res=> {
    ...
    const handleReject = (item)=>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axiosSecure.delete(`/teacherReq/${item?._id}`)
          .then(res=>{
  ...
  return (
    <div>
      ...
      <div className="  ">
        <h1 className="text-5xl text-center mt-20 font-inter font-semibold drop-shadow-lg">Total Teachers Request : {teacherReq.length}</h1>
            ...
            <tbody>
                {
                    teacherReq.map((item , index) =>   <tr  key={item._id}>
                        ...
                        <th className="flex">
                          <div>
                          <button onClick={()=>handleTeacherApprove(item)}  className="px-2 py-2 rounded text-xl bg-yellow-400 mr-3 ">Approve</button>
                          <button onClick={()=>handleReject(item)} className="px-2 py-2 rounded text-xl bg-yellow-400 " >Reject </button>
                          </div>
